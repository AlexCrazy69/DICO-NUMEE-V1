...
const ThemeMenu: React.FC<{ theme: Theme; setTheme: (theme: Theme) => void }> = ({ theme, setTheme }) => {
    ...
    const themes: { id: Theme; label: string; icon: React.ReactNode }[] = [
        { id: 'light', label: 'Clair', icon: <Sun className="w-5 h-5" /> },
        { id: 'dark', label: 'Sombre', icon: <Moon className="w-5 h-5" /> },
        { id: 'classic', label: 'Classique', icon: <Book className="w-5 h-5" /> },
        { id: 'system', label: 'Système', icon: <Laptop className="w-5 h-5" /> },
    ];
...
const Header: React.FC<HeaderProps> = ({ currentView, setView, theme, setTheme }) => {
  ...
  const navLinks = [
    ...
    { view: 'guide-touristique', label: 'Guide', icon: <Briefcase /> },
    { view: 'games', label: 'Jeux', icon: <Puzzle /> },
    { view: 'certification', label: 'Certification', icon: <Award /> },
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="md:hidden flex items-center">
              <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2 rounded-full text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
      ...
      >
        ...
        <div className="absolute right-0 top-0 h-full w-4/5 max-w-sm bg-white dark:bg-slate-800 shadow-2xl p-6 flex flex-col">
            <div className="flex justify-between items-center mb-8">
              ...
              <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700">
                <X className="w-6 h-6" />
              </button>
            </div>
